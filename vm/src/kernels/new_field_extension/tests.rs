...
use crate::{
    ...
    kernels::{
        ...
        new_field_extension::{
            NewFieldExtension, NewFieldExtensionChip, NewFieldExtensionCoreChip,
        },
    },
...
fn new_field_extension_air_test() {
    ...
    let mut chip = NewFieldExtensionChip::new(
        NativeVectorizedAdapterChip::new(
            tester.execution_bus(),
            tester.program_bus(),
            tester.memory_controller(),
        ),
        NewFieldExtensionCoreChip::new(0),
        tester.memory_controller(),
    );
    ...
    for _ in 0..num_ops {
        ...
        let result = NewFieldExtension::solve(opcode, operand1, operand2).unwrap();
...
fn new_field_extension_consistency_test() {
    ...
    for (a, b) in operands {
        ...
        let my_add = NewFieldExtension::add(a, b);
        let my_sub = NewFieldExtension::subtract(a, b);
        let my_mul = NewFieldExtension::multiply(a, b);
        let my_div = NewFieldExtension::divide(a, b);
