...
use crate::{
    keygen::v2::{types::MultiStarkVerifyingKeyV2, view::MultiStarkVerifyingKeyV2View},
    prover::{opener::AdjacentOpenedValues, v2::types::ProofV2},
    verifier::{constraints::verify_single_rap_constraints, VerificationError},
};

/// Verifies a partitioned proof of multi-matrix AIRs.
pub struct MultiTraceStarkVerifierV2<'c, SC: StarkGenericConfig> {
    config: &'c SC,
}

impl<'c, SC: StarkGenericConfig> MultiTraceStarkVerifierV2<'c, SC> {
    ...
    pub fn verify_raps(
        &self,
        challenger: &mut SC::Challenger,
        mvk: &MultiStarkVerifyingKeyV2View<SC>,
        proof: &ProofV2<SC>,
    ) -> Result<(), VerificationError> {
