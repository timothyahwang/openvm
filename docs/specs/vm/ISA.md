...
| HintFinalExp  | 9            | `a,b,_`       | Uses `b = PAIRING_IDX` to determine the curve: `BN254 = 0, BLS12-381 = 1`. Peeks at `[r32{0}(a)..r32{0}(a) + Fp::NUM_LIMBS * 12]_2` to get `f: Fp12` and then resets the hint stream to equal `final_exp_hint(f) = (residue_witness, scaling_factor): (Fp12, Fp12)` as `NUM_LIMBS * 12 * 2` bytes. |
