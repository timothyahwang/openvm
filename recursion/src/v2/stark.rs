...
use afs_stark_backend::{
    ...
    prover::{opener::AdjacentOpenedValues, v2::types::ProofV2},
};
...
use crate::{
    ...
    hints::Hintable,
    types::{AdjacentOpenedValuesVariable, CommitmentsVariable, InnerConfig},
    utils::const_fri_config,
    v2::{
        types::{MultiStarkVerificationAdviceV2, StarkVerificationAdviceV2},
        vars::{AirProofDataVariable, StarkProofV2Variable},
        view::get_advice_per_air,
    },
};

#[derive(Debug, Clone, Copy)]
pub struct VerifierProgramV2<C: Config> {
    _phantom: PhantomData<C>,
}

impl VerifierProgramV2<InnerConfig> {
    /// Create a new instance of the program for the [BabyBearPoseidon2] config.
    pub fn build(
        constants: MultiStarkVerificationAdviceV2<InnerConfig>,
    ...
    pub fn build_with_options(
        ...
        let input: StarkProofV2Variable<_> = builder.uninit();
        ProofV2::<BabyBearPoseidon2Config>::witness(&input, &mut builder);

        let pcs = TwoAdicFriPcsVariable {
            config: const_fri_config(&mut builder, fri_params),
        };
        StarkVerifierV2::verify::<DuplexChallengerVariable<_>>(
            &mut builder,
            &pcs,
            constants,
            &input,
...
#[derive(Debug, Clone, Copy)]
pub struct StarkVerifierV2<C: Config> {
    _phantom: PhantomData<C>,
}

impl<C: Config> StarkVerifierV2<C>
where
    C::F: TwoAdicField,
{
    ...
    pub fn verify_single_rap_constraints(
        builder: &mut Builder<C>,
        constants: &StarkVerificationAdviceV2<C>,
